...
def mortgage_averages(state_name):
    ...
    df1 = df1.rename(columns= {'value': 'Loan Amount'})
    ...
    final_join = join1.merge(df3, how = 'inner', on = ['Year','State','Loan Term'])

    output = StringIO()
    writer = csv.writer(output)
    writer.writerow(['Year', 'State', 'Loan Amount', 'Average Interest Rate', 'Average LTV'])

    for index, row in final_join.iterrows():
        writer.writerow([row['Year'], row['State'], row['Loan Amount'], row['Average Interest Rate'], row['Average LTV']])
